var net = require('net')
  ...
  , dns = require('dns')
   	...
   	//TODO: are these neccessary? I think i can remove them
  , createPacketBuffer = protocol.createPacketBuffer
  , parsePacket = protocol.parsePacket
  , parsePacketId = protocol.parsePacketId 
  , parsePacketLen = protocol.parsePacketLen 
...
Client.prototype.setSocket = function(socket) {
  ...
  self.socket.on('data', function(data) {
    ...
    while (true) {
      ...
      var plen=parsePacketLen(incomingBuffer);
      var pid=parsePacketId(incomingBuffer);
      
      if(pid!==null){
    	  //this event sends the same data as the parsePacket method accepts
    	  var name=protocol.packetNames[self.state][self.isServer ? 'toServer' : 'toClient'][pid.value];
    	  var slice=incomingBuffer.slice(0, plen.value+plen.size);
    	  var args= {id:pid.value, buffer:slice, state:self.state, direction:self.isServer, packetsToParse:self.packetsToParse}
    	  self.emit('parse', args);	
    	  self.emit('parse.'+pid.value, args);
    	  self.emit('parse.'+name, args);
    	  //console.log(name);
      }
      ...
      self.emit('packet', packet, packetName); 
...
Client.prototype.write = function(packetId, params) {
 ...
 //for debbuging
  this.emit('write',{id:packetId, params:params});
